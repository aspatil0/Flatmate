...
const normalizePost = (post, fallbackPostedBy = 'User') => {
  ...
  return {
    ...
    // Contact number is controlled by API - don't use owner.phone as fallback
...
const fakeFlatsData = []; // Empty array - use only local and API posts
...
const Dashboard = () => {
  ...
  const [latestNumberRequest, setLatestNumberRequest] = useState(null);
...
// Fetch posts from backend
  useEffect(() => {
    const fetchPosts = async () => {
      ...
      try {
        ...
        // If no posts from backend, use only local posts
        if (!postsArray || postsArray.length === 0) {
          postsArray = [...localPosts];
        } else {
          postsArray = [...localPosts, ...postsArray];
        }
        
        // Hide contact numbers from non-owners (privacy feature)
        // Contact numbers should only be visible to: post owner or users with approved access
        postsArray = postsArray.map(post => {
          const isOwner = post.owner?._id === user?._id || post.ownerId === user?._id;
          // Hide contact number if user is not the owner
          if (!isOwner) {
            return { ...post, contactNumber: undefined };
          }
          return post;
        });
        ...
        console.log('Using local posts only - API error or timeout:', err.message);
        const localPosts = getLocalPosts();
        // Apply privacy filtering to local posts too
        const filteredPosts = localPosts.map(post => {
          const isOwner = post.owner?._id === user?._id || post.ownerId === user?._id;
          if (!isOwner) {
            return { ...post, contactNumber: undefined };
          }
        ...
        const transformedPosts = dedupePosts(filteredPosts.map(post => normalizePost(post)));
        setPosts(transformedPosts);
      } finally {
        setLoading(false);
      }
  ...
  // Check for approved number access and update posts in real-time
  useEffect(() => {
    if (!token || !user) return;

    const checkNumberAccess = async () => {
      try {
        // Filter posts that need checking (no contact number and not owner's post)
        const postsToCheck = posts.filter(post => {
          const isOwner = post.owner?._id === user?._id || post.ownerId === user?._id;
          return !post.contactNumber && !isOwner;
        });

        if (postsToCheck.length === 0) return; // No posts to check
        ...
        const updatedPosts = await Promise.all(
          postsToCheck.map(async (post) => {
            ...
            try {
              const accessResponse = await postsAPI.checkNumberAccess(postId, token);
              if (accessResponse.hasAccess && accessResponse.contactNumber) {
                console.log(`✅ Access granted to post ${postId}, showing contact number: ${accessResponse.contactNumber}`);
                hasUpdates = true;
                return { ...post, contactNumber: accessResponse.contactNumber };
              }
            } catch (err) {
        ...
        // If any posts received approval, update the state
        if (hasUpdates) {
          console.log('📱 Updating posts with newly approved contact numbers');
          setPosts(prevPosts => {
            // Create a map of updated posts by ID for easy lookup
            const updatedMap = new Map(updatedPosts.map(post => [post.id || post._id, post]));
            // Update the full posts array, replacing any posts that were updated
            return prevPosts.map(post => updatedMap.get(post.id || post._id) || post);
          });
        }
      } catch (err) {
        console.error('Error in number access polling:', err);
      }
    };

    // Run check every 3 seconds
    const intervalId = setInterval(checkNumberAccess, 3000);
  ...
  // Fetch number requests for post owners
  useEffect(() => {
    if (!token || !user) return undefined;

    const fetchNumberRequests = async () => {
      try {
        const requests = await postsAPI.getNumberRequests(token);
        setNumberRequests(requests);

        // Show notification for first unseen pending request
        const pendingRequests = requests.filter(req => req.status === 'pending');
        if (pendingRequests.length > 0 && !latestNumberRequest) {
          setLatestNumberRequest(pendingRequests[0]);
        }
      } catch (err) {
        console.error('Error fetching number requests:', err);
      }
    };

    fetchNumberRequests();
    const intervalId = setInterval(fetchNumberRequests, 10000); // Poll every 10 seconds
  ...
  const handleGetNumber = async (post) => {
    if (!token) {
      setBookingMessage({
        type: 'error',
        text: 'Please log in to request contact number.'
      });
      setTimeout(() => setBookingMessage(null), 3000);
      return;
    }
    ...
    try {
      setBookingMessage(null);
      await postsAPI.requestContactNumber(post.id, token);
      setBookingMessage({
        type: 'success',
        text: `✅ Request sent! The property owner will respond shortly.`
      });
      setTimeout(() => setBookingMessage(null), 4000);
    } catch (err) {
      // Handle "already have access" error by fetching the contact number
      if (err.message && err.message.includes('already have access')) {
        console.log('User already has access, fetching contact number...');
        try {
          const accessResponse = await postsAPI.checkNumberAccess(post.id, token);
          if (accessResponse.hasAccess && accessResponse.contactNumber) {
            // Update the post with the contact number
            setPosts(prevPosts =>
              prevPosts.map(p =>
                (p.id === post.id || p._id === post._id) ? { ...p, contactNumber: accessResponse.contactNumber } : p
              )
            );
            setBookingMessage({
              type: 'success',
              text: `✅ Contact number: ${accessResponse.contactNumber}`
            });
            setTimeout(() => setBookingMessage(null), 5000);
          }
        } catch (accessErr) {
          console.error('Error fetching already-approved contact number:', accessErr);
          setBookingMessage({
            type: 'error',
            text: 'Contact number is approved but unable to retrieve. Please refresh.'
          });
        }
      } else {
        setBookingMessage({
          type: 'error',
          text: err.message || 'Failed to request contact number'
        });
      }
      console.error('Error requesting number:', err);
    }
  };

  const handleRespondToNumberRequest = async (requestId, status) => {
    try {
      await postsAPI.respondToNumberRequest(requestId, status, token);
      
      // Update local state
      setNumberRequests(prevRequests => 
      ...
      // Clear notification
      setLatestNumberRequest(null);
      
      setBookingMessage({
        type: 'success',
        text: `✅ Number request ${status === 'approved' ? 'approved' : 'rejected'} successfully!`
      });
      setTimeout(() => setBookingMessage(null), 3000);
    } catch (err) {
      setBookingMessage({
        type: 'error',
        text: err.message || `Failed to ${status} request`
      });
      console.error(`Error responding to number request:`, err);
    }
  };
    ...
    <div className="flex h-screen bg-gray-50 font-sans overflow-hidden">
      ...
      <main className="flex-1 overflow-y-auto custom-scrollbar bg-gray-50/50 p-6 sm:p-10 relative">
        ...
        {!loading && activeTab === 'feed' ? (
          ...
          (tenantBookingResponses.length > 0 || allOwnerNotifications.length > 0 || numberRequests.length > 0) ? (
            <div className="max-w-4xl mx-auto space-y-4">
              ...
              {numberRequests.length > 0 && (
                <div>
                  <h3 className="text-xl font-bold text-dark-900 mb-4 px-2">📞 Contact Number Requests</h3>
                    ...
                    >
                      <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
                        <div>
                          ...
                          <p className="text-lg font-semibold text-dark-900">
                            <span className="text-purple-700">{request.requesterName}</span> requested your contact number
                          </p>
                          <p className="text-sm text-gray-500 mt-2">Email: {request.requesterEmail}</p>
                          <p className="text-sm text-gray-500">Property: {request.postId?.title || 'Unknown'}</p>
                        ...
                        {request.status === 'pending' ? (
                          <div className="flex gap-3">
                            <button
                              type="button"
                              onClick={() => handleRespondToNumberRequest(request._id, 'approved')}
                              className="px-4 py-2.5 bg-green-600 text-white font-semibold rounded-xl hover:bg-green-700 transition-colors whitespace-nowrap"
                            >
                              Share Number
                            </button>
                            <button
                              type="button"
                              onClick={() => handleRespondToNumberRequest(request._id, 'rejected')}
                              className="px-4 py-2.5 bg-red-600 text-white font-semibold rounded-xl hover:bg-red-700 transition-colors whitespace-nowrap"
                            >
                              Decline
                            </button>
                          </div>
                          ...
                          >
                            {request.status === 'approved' ? '✓ Number Shared' : '✕ Request Declined'}
                          </div>
                        )}
                      </div>
                    </div>
      ...
      {latestNumberRequest && (
        <div className="fixed bottom-6 right-6 z-50 max-w-sm w-full">
          <div className="bg-white border border-purple-200 shadow-2xl rounded-2xl p-5">
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="text-xs font-bold uppercase tracking-wide text-purple-600 mb-1">📞 Number Request</p>
                <h3 className="text-lg font-bold text-dark-900">Someone wants your contact number</h3>
                <p className="text-sm text-gray-600 mt-2">
                  <span className="font-semibold">{latestNumberRequest.requesterName}</span> ({latestNumberRequest.requesterEmail}) requested your number for{' '}
                  <span className="font-semibold">{latestNumberRequest.postId?.title || 'your property'}</span>.
                </p>
                <p className="text-xs text-gray-500 mt-2">
                  {new Date(latestNumberRequest.createdAt).toLocaleString()}
                </p>
                <div className="mt-4 flex gap-2">
                  <button
                    type="button"
                    onClick={() => handleRespondToNumberRequest(latestNumberRequest._id, 'approved')}
                    className="flex-1 bg-green-600 text-white font-semibold py-2.5 rounded-xl hover:bg-green-700 transition-colors"
                  >
                    Share Number
                  </button>
                  <button
                    type="button"
                    onClick={() => handleRespondToNumberRequest(latestNumberRequest._id, 'rejected')}
                    className="flex-1 bg-red-600 text-white font-semibold py-2.5 rounded-xl hover:bg-red-700 transition-colors"
                  >
                    Decline
                  </button>
              ...
              <button
                type="button"
                onClick={() => setLatestNumberRequest(null)}
                className="w-8 h-8 rounded-full bg-gray-100 text-gray-500 hover:bg-gray-200"
              >
                ✕
              </button>

...
const buildConversationSummary = (conversation, currentUserId) => {
    const otherParticipant = (conversation.participants || []).find((participant) => participant._id?.toString() !== currentUserId);
    const unreadCount = (conversation.messages || []).filter((message) => message.sender?.toString() !== currentUserId &&
        !(message.readBy || []).some((reader) => reader?.toString() === currentUserId)).length;
    return {
        id: conversation._id,
...
export const getConversations = async (req, res) => {
    try {
        ...
        return res.json({
            conversations: conversations.map((conversation) => buildConversationSummary(conversation, currentUserId)),
        });
    }
    catch (error) {
...
export const getConversationMessages = async (req, res) => {
    try {
        ...
        const isParticipant = conversation.participants.some((participant) => participant._id.toString() === currentUserId);
        if (!isParticipant) {
            return res.status(403).json({ error: 'You are not allowed to view this conversation' });
        }
        let hasUnreadUpdates = false;
        conversation.messages.forEach((message) => {
            const alreadyRead = (message.readBy || []).some((reader) => reader.toString() === currentUserId);
            if (!alreadyRead) {
